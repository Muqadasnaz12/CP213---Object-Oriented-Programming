...
public class SingleList<T extends Comparable<T>> extends SingleLink<T> {
    ...
    public void clean() {
    	...
    	while (current != null) {
    		...
    		if (runner.getNext().getEntity().compareTo(current.getEntity()) == 0) {
    ...
    public T remove(final T key) {
    	...
    	if (this.front.getEntity().compareTo(key) == 0) {
    	    ...
    	    this.length -= 1;
    	    return removedValue.getEntity();
    	}
    	SingleNode<T> current = this.front;
    	while (current.getNext() != null) {
    	    if (current.getNext().getEntity().compareTo(key) == 0) {
    ...
    public void union(final SingleList<T> left, final SingleList<T> right) {
    	...
    	while (currLeft != null) {
    	    this.append(currLeft.getEntity());
    	    currLeft = currLeft.getNext();
    	}
    	while (currRight != null) {
    	    this.append(currRight.getEntity());
