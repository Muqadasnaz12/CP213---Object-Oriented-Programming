...
public class SinglePriorityQueue<T extends Comparable<T>> extends SingleLink<T> {
	...
	while (currLeft != null && currRight != null) {
	    compare = currLeft.getEntity().compareTo(currRight.getEntity());
	    if (compare <= 0) {
    ...
    public void insert(final T entity) {
    	...
    	while (current != null && current.getEntity().compareTo(entity) <= 0) {
    	    prev = current;
    	    current = current.getNext();
    	}
    	SingleNode<T> node = new SingleNode<>(entity, current);
