 ...
 *
 * @author Muqadas Nazif, 169061218, nazi1218@mylaurier.ca
 * @author David Brown
 * @version 2025-01-05
...
public class PopularityTree<T extends Comparable<T>> extends BST<T> {
    ...
    private TreeNode<T> retrieveAux(TreeNode<T> node, final CountedEntity<T> key) {
    	...
    	if (compare == 0) {
    		...
    		if (node.getCountedEntity().getCount() < foundNode.getCountedEntity().getCount()) {
    		    ...
    		    if (node.getCountedEntity().compareTo(this.root.getCountedEntity()) == 0) {
    			this.root = newRoot;
    		    }
    		    node.updateHeight();
    ...
    protected boolean isValidAux(final TreeNode<T> node, TreeNode<T> minNode, TreeNode<T> maxNode) {
    	...
    	if (((minNode != null && node.getCountedEntity().compareTo(minNode.getCountedEntity()) <= 0)
    		...
    		|| (node.getCountedEntity().getCount() < leftCount) || (node.getCountedEntity().getCount() < rightCount)) {
    	    return false;
    	}
    ...
    public CountedEntity<T> retrieve(CountedEntity<T> key) {
    	...
    	if (foundNode != null) {
    	    return foundNode.getCountedEntity();
    	}
    	return null;
    }
}
